...
<script runat="server">
    ...
    Sub BindGrid()
        Using connection As New OleDb.OleDbConnection(connectionString)
            Dim query As String = "SELECT id, soal FROM dbo.t_angket_soal WHERE jns = 'DSN'"
    ...
    Sub GridView2_RowUpdating(sender As Object, e As GridViewUpdateEventArgs)
        ...
        Using connection As New OleDb.OleDbConnection(connectionString)
            connection.Open()
            Dim query As String = "UPDATE dbo.t_angket_soal SET soal = ? WHERE id = ? AND jns = 'DSN'"
    ...
    Sub GridView2_RowDeleting(sender As Object, e As GridViewDeleteEventArgs)
        ...
        Using connection As New OleDb.OleDbConnection(connectionString)
            connection.Open()
            Dim query As String = "DELETE FROM dbo.t_angket_soal WHERE id = ? AND jns = 'DSN'"
            Using command As New OleDb.OleDbCommand(query, connection)
                command.Parameters.AddWithValue("?", id)
                command.ExecuteNonQuery()
            End Using
    ...
    Sub btnAdd_Click(sender As Object, e As EventArgs)
        Dim jnsValue As String = "DSN" ' sesuaikan sesuai kebutuhan
