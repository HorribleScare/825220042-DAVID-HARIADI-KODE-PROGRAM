...
<script runat="server">
    ...
    Sub LoadDashboardData()
        ...
        Using connection As New OleDb.OleDbConnection(connectionString)
            ...
            Dim query As String = "SELECT COUNT(*) AS TotalResponden FROM tqvm_hslangket"
            ...
            Dim query2 As String = "SELECT COUNT(*) AS TotalMHS FROM tqvm_hslangket WHERE sts = 'MHS'"
            ...
            Dim query3 As String = "SELECT COUNT(*) AS TotalDSN FROM tqvm_hslangket WHERE sts = 'DSN'"
            ...
            Dim query4 As String = "SELECT COUNT(*) AS TotalKRY FROM tqvm_hslangket WHERE sts = 'KRY'"
            ...
            ' Query untuk mengambil semua isi dari tqvm_hslangket
            Dim queryIsi As String = "SELECT isi FROM tqvm_hslangket"
                ...
                Using reader As OleDb.OleDbDataReader = command.ExecuteReader()
                    While reader.Read()
                        ...
                        If Not String.IsNullOrEmpty(isiRaw) Then
                            ...
                            For i As Integer = 0 To isiRaw.Length - 1
                                Dim ch As Char = isiRaw(i)
            ...
            ' Query untuk mengambil isi dari tqvm_hslangket dengan sts = 'MHS'
            Dim queryIsiMahasiswa As String = "SELECT isi FROM dbo.tqvm_hslangket WHERE sts = 'MHS'"
            ...
            ' Query untuk mengambil isi dari tqvm_hslangket dengan sts = 'DSN'
            Dim queryIsiDosen As String = "SELECT isi FROM dbo.tqvm_hslangket WHERE sts = 'DSN'"
            ...
            ' Query untuk mengambil isi dari tqvm_hslangket dengan sts = 'KRY'
            Dim queryIsiKaryawan As String = "SELECT isi FROM dbo.tqvm_hslangket WHERE sts = 'KRY'"
    ...
    Sub LoadChartData()
        Dim chartData As New List(Of Object) From {
            ...
            New With {.ChartId = "distribusiChart", .ChartTitle = "Distribusi Skor Jawaban Setiap Pertanyaan (2016-2022)", .ChartType = "bar", .Width = "12"},
            New With {.ChartId = "distribusiChartStatusMahasiswa", .ChartTitle = "Distribusi Skor Jawaban Mahasiswa (2016-2022)", .ChartType = "bar", .Width = "12"},
            New With {.ChartId = "distribusiChartStatusDosen", .ChartTitle = "Distribusi Skor Jawaban Dosen (2016-2022)", .ChartType = "bar", .Width = "12"},
            New With {.ChartId = "distribusiChartStatusKaryawan", .ChartTitle = "Distribusi Skor Jawaban Karyawan (2016-2022)", .ChartType = "bar", .Width = "12"}
        }
    ...
    Sub GenerateChartConfiguration()
        ...
        ' Distribusi Skor Jawaban Setiap Pertanyaan (2016-2022)
        Dim distribusi(4, 4) As Integer ' 5 pertanyaan × skor 1–5
        Using connection As New OleDb.OleDbConnection(connectionString)
            connection.Open()
            Dim cmd As New OleDb.OleDbCommand("SELECT isi FROM dbo.tqvm_hslangket WHERE isi IS NOT NULL", connection)
            Dim reader As OleDb.OleDbDataReader = cmd.ExecuteReader()
            While reader.Read()
            Dim isi As String = reader("isi").ToString().Trim()
            For i As Integer = 0 To Math.Min(isi.Length, 5) - 1
                Dim skor As Integer
                If Integer.TryParse(isi(i).ToString(), skor) Then
        ...
        ' Labels pertanyaan
        Dim labelsDistribusi As String = "Pertanyaan 1,Pertanyaan 2,Pertanyaan 3,Pertanyaan 4,Pertanyaan 5"

        ' Dataset per skor 1–5
        ...
        ' Distribusi Skor Jawaban Mahasiswa (2016-2022)
        Dim distribusiMahasiswa(4, 4) As Integer ' 5 pertanyaan × skor 1–5
        Using connection As New OleDb.OleDbConnection(connectionString)
            connection.Open()
            Dim cmd As New OleDb.OleDbCommand("SELECT isi FROM dbo.tqvm_hslangket WHERE isi IS NOT NULL AND sts = 'MHS'", connection)
            Dim reader As OleDb.OleDbDataReader = cmd.ExecuteReader()
            While reader.Read()
            Dim isi As String = reader("isi").ToString().Trim()
            For i As Integer = 0 To Math.Min(isi.Length, 5) - 1
                ...
                If Integer.TryParse(isi(i).ToString(), skor) Then
                If skor >= 1 AndAlso skor <= 5 Then
                    distribusiMahasiswa(i, skor - 1) += 1
                End If
                End If
            Next
            End While
        ...
        ' Labels pertanyaan
        Dim labelsDistribusiMahasiswa As String = "Pertanyaan 1,Pertanyaan 2,Pertanyaan 3,Pertanyaan 4,Pertanyaan 5"

        ' Dataset per skor 1–5
        Dim dataPartsMahasiswa As New List(Of String)
        For skor As Integer = 1 To 5
            Dim values As New List(Of String)
            For q As Integer = 0 To 4
            values.Add(distribusiMahasiswa(q, skor - 1).ToString())
            Next
            dataPartsMahasiswa.Add("Skor " & skor & ":" & String.Join(",", values))
        Next

        Dim distribusiDataMahasiswa As String = String.Join("|", dataPartsMahasiswa)

        ' Tambahkan chart distribusi ke chartDetails
        chartDetails.Add(New With {
            .id = "distribusiChartStatusMahasiswa",
            .type = "bar",
            .labels = labelsDistribusiMahasiswa,
            .data = distribusiDataMahasiswa,
            .isMulti = True,
            .showLegend = True
        })
        ...
        ' Distribusi Skor Jawaban Dosen (2016-2022)
        Dim distribusiDosen(4, 4) As Integer ' 5 pertanyaan × skor 1–5
        Using connection As New OleDb.OleDbConnection(connectionString)
            connection.Open()
            Dim cmd As New OleDb.OleDbCommand("SELECT isi FROM dbo.tqvm_hslangket WHERE isi IS NOT NULL AND sts = 'DSN'", connection)
            Dim reader As OleDb.OleDbDataReader = cmd.ExecuteReader()
            While reader.Read()
            Dim isi As String = reader("isi").ToString().Trim()
            For i As Integer = 0 To Math.Min(isi.Length, 5) - 1
        ...
        ' Labels pertanyaan
        Dim labelsDistribusiDosen As String = "Pertanyaan 1,Pertanyaan 2,Pertanyaan 3,Pertanyaan 4,Pertanyaan 5"

        ' Dataset per skor 1–5
        Dim dataPartsDosen As New List(Of String)
        For skor As Integer = 1 To 5
            Dim values As New List(Of String)
            For q As Integer = 0 To 4
            values.Add(distribusiDosen(q, skor - 1).ToString())
            Next
            dataPartsDosen.Add("Skor " & skor & ":" & String.Join(",", values))
        Next

        Dim distribusiDataDosen As String = String.Join("|", dataPartsDosen)

        ' Tambahkan chart distribusi ke chartDetails
        chartDetails.Add(New With {
            .id = "distribusiChartStatusDosen",
            .type = "bar",
            .labels = labelsDistribusiDosen,
            .data = distribusiDataDosen,
            .isMulti = True,
            .showLegend = True
        })
        ...
        ' Distribusi Skor Jawaban Karyawan (2016-2022)
        Dim distribusiKaryawan(4, 4) As Integer ' 5 pertanyaan × skor 1–5
        Using connection As New OleDb.OleDbConnection(connectionString)
            connection.Open()
            Dim cmd As New OleDb.OleDbCommand("SELECT isi FROM dbo.tqvm_hslangket WHERE isi IS NOT NULL AND sts = 'KRY'", connection)
            Dim reader As OleDb.OleDbDataReader = cmd.ExecuteReader()
            While reader.Read()
            Dim isi As String = reader("isi").ToString().Trim()
            For i As Integer = 0 To Math.Min(isi.Length, 5) - 1
        ...
        ' Labels pertanyaan
        Dim labelsDistribusiKaryawan As String = "Pertanyaan 1,Pertanyaan 2,Pertanyaan 3,Pertanyaan 4,Pertanyaan 5"

        ' Dataset per skor 1–5
        Dim dataPartsKaryawan As New List(Of String)
        For skor As Integer = 1 To 5
            Dim values As New List(Of String)
            For q As Integer = 0 To 4
            values.Add(distribusiKaryawan(q, skor - 1).ToString())
            Next
            dataPartsKaryawan.Add("Skor " & skor & ":" & String.Join(",", values))
        Next

        Dim distribusiDataKaryawan As String = String.Join("|", dataPartsKaryawan)

        ' Tambahkan chart distribusi ke chartDetails
        chartDetails.Add(New With {
            .id = "distribusiChartStatusKaryawan",
            .type = "bar",
            .labels = labelsDistribusiKaryawan,
            .data = distribusiDataKaryawan,
            .isMulti = True,
            .showLegend = True
        })
...
<section class="content-header">
    ...
    <div class="box-footer text-center" style="margin-top: 15px;">
        <asp:LinkButton ID="btnViewAll" runat="server" CssClass="btn btn-default" OnClick="btnViewAll_Click">
            Data Lengkap Tahunan Visi Misi
        </asp:LinkButton>
    </div>
</section>
    ...
    <div class="modal-dialog" style="width:850px;">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="modalLabel">Data Lengkap Tahunan Visi Misi</h4>
            ...
            <div class="modal-body">
                <!-- Dropdown Tahun -->
                <asp:DropDownList ID="ddlTahun" runat="server" AutoPostBack="True" OnSelectedIndexChanged="ddlTahun_SelectedIndexChanged" CssClass="form-control" style="width: 200px; margin-bottom: 10px;">
                </asp:DropDownList>

                <!-- Dropdown Prodi -->
                <asp:DropDownList ID="ddlProdi" runat="server" AutoPostBack="True" OnSelectedIndexChanged="ddlProdi_SelectedIndexChanged" CssClass="form-control" style="width: 300px; margin-bottom: 10px;">
                </asp:DropDownList>
                ...
                <asp:GridView ID="GridViewTabelTahunan" runat="server" CssClass="table table-bordered table-striped" AutoGenerateColumns="False">
                    <Columns>
                        <asp:BoundField DataField="thn" HeaderText="Tahun" />
...
<script runat="server">
    ...
    Protected Sub btnViewAll_Click(sender As Object, e As EventArgs)
        ...
        ' Set default pilihan tahun ke yang pertama (misal tahun terkecil atau terbesar)
    ...
    Protected Sub ddlTahun_SelectedIndexChanged(sender As Object, e As EventArgs)
        ...
        ' Tampilkan modal lagi setelah postback
        ScriptManager.RegisterStartupScript(Me, Me.GetType(), "ShowModal", "$('#modal-view-all').modal('show');", True)
    End Sub

    Private Sub LoadTahunDropdown()
        Dim query As String = "SELECT DISTINCT thn FROM dbo.tqvm_hslangket ORDER BY thn"
        ...
        Using connection As New System.Data.OleDb.OleDbConnection(connectionString)
            Using command As New System.Data.OleDb.OleDbCommand(query, connection)
                ...
                While reader.Read()
                    Dim yearValue As String = reader("thn").ToString()
    ...
    Private Sub LoadProdiDropdown()
        ...
        ddlProdi.Items.Add(New ListItem("Semua Prodi", "")) ' opsional, default

        Dim prodiMap As New Dictionary(Of String, String) From {
            {"115", "Ekonomi Manajemen"},
            {"125", "Ekonomi Akuntansi"},
            {"127", "Magister Akuntansi"},
            {"117", "Magister Manajemen"},
            {"205", "Hukum"},
            {"207", "Magister Ilmu Hukum"},
            {"315", "Teknik Arsitektur"},
            {"325", "Teknik Sipil"},
            {"347", "Magister Teknik Perencanaan"},
            {"345", "Teknik Perencanaan Wilayah & Kota"},
            {"405", "Kedokteran"},
            {"406", "Profesi Dokter"},
            {"515", "Teknik Mesin"},
            {"525", "Teknik Elektro"},
            ...
            {"103", "Dosen (Kode 103)"},
            {"107", "Dosen (Kode 107)"},
            {"200", "Karyawan (Kode 200)"},
            {"201", "Karyawan (Kode 201)"},
            {"202", "Karyawan (Kode 202)"},
            {"203", "Karyawan (Kode 203)"},
            {"204", "Karyawan (Kode 204)"},
            {"206", "Karyawan (Kode 206)"},
            {"208", "Karyawan (Kode 208)"},
            {"209", "Karyawan (Kode 209)"},
            {"800", "Karyawan (Kode 800)"}
        }
    ...
    Private Sub LoadGridViewData(tahun As String)
        ...
        Dim query As String = "SELECT thn, id_user, sts, isi FROM dbo.tqvm_hslangket WHERE thn = ?"

        If Not String.IsNullOrEmpty(selectedProdi) Then
            query &= " AND LEFT(id_user, 3) = ?"
        End If
        ...
        Using connection As New System.Data.OleDb.OleDbConnection(connectionString)
            Using command As New System.Data.OleDb.OleDbCommand(query, connection)
                ...
                ' Tambah kolom Rerata
                If Not dt.Columns.Contains("Rerata") Then
                    dt.Columns.Add("Rerata", GetType(Double))
                End If
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    Dim rerata As Double = HitungRerataDigit(isiString)
                    Dim total As Double = HitungTotalDigit(isiString)

                    row("Rerata") = rerata
                    row("Total") = total
                Next

                ' === Hitung total responden dan per status ===
                Dim totalResponden As Integer = dt.Rows.Count

                Dim statusCounts = dt.AsEnumerable().
                    GroupBy(Function(r) r.Field(Of String)("sts")).
                    ToDictionary(Function(g) g.Key, Function(g) g.Count())

                ' Buat teks per status
                Dim perStatusText As String = String.Join(" | ", statusCounts.Select(Function(kvp) kvp.Key & ": " & kvp.Value))

                lblTotalResponden.Text = "Total Responden: " & totalResponden
                lblPerStatus.Text = "Per Status: " & perStatusText
    ...
    Private Function HitungRerataDigit(text As String) As Double
        ...
        For Each ch As Char In text
            If Char.IsDigit(ch) Then
                digits.Add(Convert.ToInt32(Char.GetNumericValue(ch)))
            End If
        Next

        If digits.Count = 0 Then Return 0
        Return Math.Round(digits.Average(), 2) ' Bulatkan 2 angka di belakang koma
    End Function
    ...
    Private Function HitungTotalDigit(text As String) As Double
        ...
        For Each ch As Char In text
            If Char.IsDigit(ch) Then
                total += Convert.ToInt32(Char.GetNumericValue(ch))
            End If
        Next

        Return Math.Round(total, 2) ' bulatkan dua angka di belakang koma
    End Function
